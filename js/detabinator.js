/**
 * Detabinator.js 
 * Stop event listener for focusable element
 */

class Detabinator {
    constructor(element) {
        if (!element) {
            throw new Error("Element focusable tidak ditemukan pada dokumen");
        }
        this._inert = false;
        this._focusableElementsString = 'a[href], area[href], input:not([disabled]), textarea:not([disabled]), select:not([disabled]), [tabindex], [contenteditable]';
